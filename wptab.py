...
def get_tab(url, t_vals, search="head", t_id=0, wikitable_class='wikitable sortable'):
    ...
    Each nested list represents a table row, including the header.
    Tables of a predefined html class (default 'wikitable sortable') are searched from top to bottom.
    Find a specific table requires a set object of target values (t_vals).
    Those values can be looked up in each header (default), 
    or in a particular row / column of each table (search='row' / search='column').
    The latter options require a row / column id (t_id, default=0). Ids are zero based.
    For example, search="row" and t_id=2 will go through the third row of each table.
    '''
    ...
    for table in soup.find_all('table', class_= wikitable_class):
        ...
        if search == "head":
            x = (h.text.strip() for h in table.find_all('th')) #strip() required for parsing as source code might contain additional whitespace + newline chars
        elif search == "row":
            try:
                rows = table.find_all('tr')[1:] #first row excluded as it is usually empty (containing th, but not td tags)
        ...
        ## if target values are subset of extracted data, keep table and break loop 
